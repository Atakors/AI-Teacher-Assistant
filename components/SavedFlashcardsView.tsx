  ...
  return (
    <div className="w-full max-w-6xl mx-auto">
      <div className="text-center mb-8">
        ...
        <p className="text-[var(--color-text-secondary)] mt-2">Access and manage your previously generated flashcards.</p>
      </div>
            ...
            {savedFlashcards.map(flashcard => (
              <div key={flashcard.id} className="aurora-card p-4 flex flex-col justify-between gap-4">
                <div>
                  ...
                  <h3 className="font-semibold text-lg text-[var(--color-text-primary)] truncate">{flashcard.name}</h3>
                   <p className="text-xs text-[var(--color-text-secondary)] mt-1 italic truncate">
                    Prompt: "{flashcard.prompt}"
                  </p>
                  <p className="text-xs text-[var(--color-text-secondary)] mt-1">Saved on: {new Date(flashcard.createdAt).toLocaleDateString()}</p>
                </div>
                <div className="flex-shrink-0 flex items-center gap-2 w-full">
                  <button onClick={() => onLoadFlashcard(flashcard)} className="blueprint-button py-2 px-4 rounded-lg text-sm w-full">Load</button>
