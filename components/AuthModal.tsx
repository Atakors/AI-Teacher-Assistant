...
const AuthModal: React.FC<AuthModalProps> = ({ isOpen, onClose, initialView, initialError }) => {
  ...
  return (
    <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-[100] p-4" onClick={onClose}>
      <div className="relative w-full max-w-md bg-[var(--color-surface)] rounded-xl shadow-2xl text-[var(--color-text-primary)] overflow-hidden auth-modal" onClick={e => e.stopPropagation()}>
          ...
          {isLoading ? <LoadingSpinner text={view === 'login' ? 'Logging in...' : 'Creating profile...'} /> : successMessage ? (
             <div className="text-center">
                ...
                <p className="text-[var(--color-text-secondary)] mt-2">{successMessage}</p>
                 <button onClick={onClose} className="w-full mt-6 py-3 font-semibold rounded-lg zenith-button">
                    Close
                </button>
            </div>
          ) : (
            <>
                <div className="text-center mb-6">
                    <SparklesIcon className="w-10 h-10 mx-auto text-[var(--color-accent)]" />
                    <h2 className="text-2xl font-bold mt-2">
                        {view === 'login' ? 'Welcome Back' : 'Create Your Profile'}
                    </h2>
                </div>
                <div className="flex flex-col items-center space-y-4">
                    <button onClick={handleGoogleSignIn} className="flex items-center justify-center w-full max-w-sm p-3 border border-[var(--color-border)] rounded-lg hover:bg-[var(--color-inset-bg)] transition-colors font-medium">
                    ...
                    <div className="flex items-center w-full">
                        ...
                        <span className="px-2 text-xs text-[var(--color-text-secondary)] uppercase">Or</span>
                        <hr className="w-full border-t border-[var(--color-border)]" />
                    </div>
                    <form onSubmit={handleEmailPasswordSubmit} className="w-full max-w-sm mx-auto">
                        ...
                        <button type="submit" disabled={isFormInvalid} className="w-full mt-6 py-3 font-semibold rounded-lg zenith-button">
