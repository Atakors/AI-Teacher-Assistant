...
const AuthModal: React.FC<AuthModalProps> = ({ isOpen, onClose, initialView, initialError }) => {
  ...
  const inputClasses = "w-full p-3 rounded-lg placeholder-[var(--color-text-secondary)] focus:outline-none focus:ring-2 focus:ring-[var(--color-accent)] border border-[var(--color-border)]";

  return (
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-[100] p-4" onClick={onClose}>
      <div 
        className="relative w-full max-w-md rounded-xl shadow-2xl overflow-hidden auth-modal" 
        ...
        <div className="p-8">
          {isLoading ? <LoadingSpinner text={view === 'login' ? 'Logging in...' : 'Creating profile...'} /> : successMessage ? (
             <div className="text-center">
                ...
                <p className="text-[var(--color-text-secondary)] mt-2">{successMessage}</p>
            ...
            <>
                ...
                <div className="flex flex-col items-center space-y-4">
                    <button onClick={handleGoogleSignIn} className="flex items-center justify-center w-full max-w-sm p-3 border border-[var(--color-border)] rounded-lg hover:bg-[var(--color-inset-bg)] transition-colors">
                    ...
                    <div className="flex items-center w-full">
                        ...
                        <span className="px-2 text-xs text-[var(--color-text-secondary)]">OR</span>
                        <hr className="w-full border-t border-[var(--color-border)]" />
                    </div>
                    <form onSubmit={handleEmailPasswordSubmit} className="w-full max-w-sm mx-auto">
                        <div className="space-y-4">
                            {view === 'signup' && (
                                <input type="text" placeholder="Full Name" value={name} onChange={e => setName(e.target.value)} className={inputClasses} style={{ backgroundColor: 'var(--color-input-bg)' }} required />
                            )}
                            <input type="email" placeholder="Email Address" value={email} onChange={e => setEmail(e.target.value)} className={inputClasses} style={{ backgroundColor: 'var(--color-input-bg)' }} required />
                            <input type="password" placeholder="Password" value={password} onChange={e => setPassword(e.target.value)} className={inputClasses} style={{ backgroundColor: 'var(--color-input-bg)' }} required />
                            {view === 'signup' && (
                                <input type="password" placeholder="Confirm Password" value={confirmPassword} onChange={e => setConfirmPassword(e.target.value)} className={inputClasses} style={{ backgroundColor: 'var(--color-input-bg)' }} required />
                            )}
                        </div>
