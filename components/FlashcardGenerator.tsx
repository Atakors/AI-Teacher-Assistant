...
import { generateFlashcardImageWithGemini } from '../services/geminiService';
import LoadingSpinner from './LoadingSpinner';
import ErrorMessage from './ErrorMessage';
import ImagePreviewModal from './ImagePreviewModal';
import { SparklesIcon, FlashcardIcon as ImageIcon, DownloadIcon } from '../constants';
...
const IMAGE_STYLES = [
  { name: 'Default', value: '' },
  { name: 'Cartoon', value: ', in a cute cartoon style' },
  { name: 'Watercolor', value: ', watercolor painting style' },
  { name: 'Line Art', value: ', simple black and white line art' },
  { name: 'Pixel Art', value: ', 8-bit pixel art style' },
  { name: 'Photorealistic', value: ', photorealistic' },
  { name: 'Minimalist', value: ', minimalist, simple shapes' },
  { name: '3D Render', value: ', cute 3D render' },
];
...
const FlashcardGenerator: React.FC<FlashcardGeneratorProps> = ({ onGenerate }) => {
            ...
            <div className="aurora-card p-6 sm:p-8 space-y-6 sticky top-8">
              <div>
                ...
                <textarea
                  ...
                  placeholder="e.g., a smiling cartoon apple"
                />
                <p className="mt-1 text-xs text-[var(--color-text-secondary)]">Keep prompts simple and descriptive.</p>
              </div>
                  ...
                  {ASPECT_RATIO_OPTIONS.map((option) => (
                    <label key={option.value} className="flex items-center space-x-2 cursor-pointer text-sm text-[var(--color-text-primary)] p-2 rounded-lg hover:bg-[var(--color-inset-bg)]">
                      <input
                        ...
                        onChange={() => setSelectedAspectRatio(option.value)}
              ...
              <button
                ...
                style={{ backgroundColor: 'var(--color-accent)', color: 'white', borderColor: 'transparent' }}
              >
                {isLoading ? 'Generating your image...' : 'Generate Image'}
              </button>

               {generatedImage && !isLoading && (
                <button type="button" onClick={handleDownloadImage} className={`${buttonClasses} bg-[var(--color-surface)] border border-[var(--color-border)]`}>
          ...
          <div className="lg:w-3/5 w-full mt-8 lg:mt-0">
            <div className={`p-2 sm:p-4 flex items-center justify-center w-full ${currentAspectRatioClass} overflow-hidden transition-all duration-300 ease-in-out rounded-lg`} style={{backgroundColor: 'var(--color-inset-bg)'}}>
              ...
              {!isLoading && !error && generatedImage && (
                ...
                >
                    <img src={generatedImage} alt={prompt || 'Generated image'} className="max-w-full max-h-full object-contain rounded-md" />
                </button>
              )}
              {!isLoading && !error && !generatedImage && (
