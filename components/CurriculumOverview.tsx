...
const YearlyPlanView: React.FC<{ sequences: CanvasSequence[] }> = ({ sequences }) => {
    if (!sequences || sequences.length === 0) {
        return <p className="text-[var(--color-text-secondary)] text-center py-8">Select a year to see the yearly plan.</p>;
    }
    return (
        <div className="space-y-6">
            {sequences.map((sequence) => (
                <div key={sequence.id} className="aurora-card p-6">
                    <h3 className={`text-xl font-semibold mb-3`} style={{ color: sequence.isPause ? '#f59e0b' : 'var(--color-accent)' }}>
                    ...
                    {sequence.isPause ? (
                        <>
                            ...
                            {sequence.objectives && <div className="mb-4">
                                <h4 className="text-md font-semibold text-[var(--color-text-primary)] mb-1">Objectives:</h4>
                                <ul className="list-disc list-inside ml-4 space-y-1 text-sm text-[var(--color-text-secondary)]">{sequence.objectives.map((o, i) => <li key={i}>{o}</li>)}</ul>
                            </div>}
                            {sequence.pauseDetails && <div className="mb-1 text-sm text-[var(--color-text-secondary)]">
                        ...
                        <>
                            ...
                            {sequence.sections && <div className={sequence.objectives ? "pt-4 border-t border-[var(--color-border)]" : ""}>
                                <h4 className="text-md font-semibold text-[var(--color-text-primary)] mb-2">Sections:</h4>
                                <ul className="list-disc list-inside ml-4 space-y-1 text-sm text-[var(--color-text-secondary)]">{sequence.sections.map(s => <li key={s.id}>{s.name}</li>)}</ul>
                            </div>}
                            {!sequence.objectives && !sequence.sections && <p className="text-sm italic mt-3 text-[var(--color-text-secondary)]">No details.</p>}
                        </>
                    )}
                </div>
            ))}
...
const MonthlyDistributionView: React.FC<{ sequences: CanvasSequence[], academicMonths: string[] }> = ({ sequences, academicMonths }) => {
    ...
    if (!sequences || sequences.length === 0) return <p className="text-[var(--color-text-secondary)] text-center py-8">Select a year.</p>;
    return (
        <div className="space-y-8">
            {academicMonths.map(month => (
                <div key={month} className="aurora-card p-6">
                    <h3 className="text-xl font-semibold mb-4" style={{ color: 'var(--color-accent)' }}>{month}</h3>
                    {distribution[month]?.length > 0 ? (
                        <ul className="space-y-3">
                            {distribution[month].map(seq => (
                                <li key={`${month}-${seq.id}`} className="p-3 rounded-lg border-l-4" style={{backgroundColor: 'var(--color-inset-bg)', borderColor: seq.isPause ? '#f59e0b' : 'var(--color-accent)'}}>
                                    <p className="font-medium text-[var(--color-text-primary)]">{seq.isPause ? seq.pauseTitle || seq.title : seq.title}</p>
                                    {!seq.isPause && <p className="text-xs text-[var(--color-text-secondary)] mt-1">Sections: {seq.sections.map(s => s.name).join(', ')}</p>}
                                </li>
                            ))}
                        </ul>
                    ) : <p className="text-sm text-[var(--color-text-secondary)]">No sequences planned.</p>}
                </div>
            ))}
        </div>
    );
...
const CurriculumOverview: React.FC<CurriculumOverviewProps> = ({ curriculumDataMap }) => {
  ...
  const selectClasses = "mt-1 block w-full sm:w-auto max-w-xs p-2 text-base focus:outline-none rounded-lg border border-[var(--color-border)]";
  ...
  return (
    <div className="w-full max-w-6xl mx-auto">
      <div className="text-center mb-8">
        ...
        <p className="text-[var(--color-text-secondary)] mt-2 px-4">Explore yearly, monthly, and detailed weekly plans.</p>
      </div>

      <div className="aurora-card p-6 sm:p-8 mb-8 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
            <label htmlFor="year-select-overview" className="block text-lg font-medium mb-2 text-[var(--color-text-primary)]">Academic Year:</label>
            <select
        ...
        {activeSubView === 'detailedMonthly' && selectedYear !== CurriculumLevel.SELECT_YEAR && (
            <div>
                <label htmlFor="month-select-detailed" className="block text-lg font-medium mb-2 text-[var(--color-text-primary)]">Month:</label>
        ...
        <>
          ...
          {activeSubView === 'detailedMonthly' && (
              selectedMonthForDetailedView === "September" ? (
                  <div className="aurora-card text-center py-10"><p>The Detailed Monthly Plan is not applicable for September.</p></div>
              ) : (
         ...
         <div className="aurora-card text-center py-10"><p>Please select an academic year.</p></div>
      )}
    </div>
  );
};
